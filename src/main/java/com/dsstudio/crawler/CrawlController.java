...
public class CrawlController extends Configurable {
	...
	protected <T extends KeywordCrawler> void start(final KeywordCrawlerFactory<T> crawlerFactory,
		...
		try {
			 ...
			 * synchronized(waitingLock){ while(true){
			 * sleep(config.getThreadMonitoringDelaySeconds()); boolean
			 * someoneIsWorking = false; for(int i = 0; i < threads.size();
			 * i++){ Thread thread = threads.get(i); if(!thread.isAlive()){
			 * logger.info("Thread {} was dead, I'll recreate it", i); T crawler
			 * = crawlerFactory.newInstance(); thread = new Thread(crawler,
			 * "Crawler " + (i + 1)); threads.remove(i); threads.add(i, thread);
			 * crawler.setThread(thread); crawler.init(i + 1, controller);
			 * thread.start(); crawlers.remove(i); crawlers.add(i, crawler); } }
			 * waitingLock.notifyAll(); } } }catch(Exception e){
			 * e.printStackTrace(); } }
			 * 
			 * });
			 * 
